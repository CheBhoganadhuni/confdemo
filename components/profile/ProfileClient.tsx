...
interface CommitDialogProps {
  taskId: string
  lockedUsername: string | null   // pre-filled + read-only if github connected
  onSuccess: () => void
  onTokenEarned: () => void
}
...
function CommitDialog({ taskId, lockedUsername, onSuccess, onTokenEarned }: CommitDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!u || !r || !s) {
      setDialogError('All three fields are required.')
      return
    }
    ...
    try {
      ...
      try {
        ...
        setDialogError('Server returned an unexpected response. Try again.')
        return
      }
      ...
      if (!res.ok) {
        setDialogError(
          (data.message as string) ??
            'Verification failed. Check your inputs and try again.'
        )
        return
      }
      ...
      if (data.token_earned) {
        onTokenEarned()
        toast.success('⚡ Bolt earned! Great work today.', { duration: 4000 })
      } else {
  ...
  // Reset error when any field changes
          ...
          <div className="flex flex-col gap-4 mt-2">
              ...
              {lockedUsername ? (
                <div className="relative">
                  ...
                  <p className="text-[10px] text-[#444] mt-1 flex items-center gap-1">
                    ...
                    Connected account — cannot change
                  </p>
                </div>
              ) : (
                <input
                  type="text"
                  value={ghUser}
                  onChange={field(setGhUser)}
                  placeholder="e.g. chebhoganadhuni"
            ...
            <div>
              ...
              <input
                type="text"
                value={ghRepo}
                onChange={field(setGhRepo)}
                placeholder="e.g. confdemo"
                className="w-full h-9 bg-[#0A0A0A] border border-[#1F1F1F] text-white text-sm px-3 rounded-sm focus:outline-none focus:border-[#F97316]/60 placeholder:text-[#333]"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                value={ghSha}
                onChange={field(setGhSha)}
                placeholder="e.g. 6d0602da52..."
            ...
            {/* Error */}
            {dialogError && (
              <p className="text-rose-400 text-xs leading-relaxed">{dialogError}</p>
            )}
...
export function ProfileClient({
  ...
  // Generic task complete handler (for non-github tasks)
  const handleTaskComplete = async (taskId: string, proofUrl: string) => {
    ...
    try {
      ...
      setTaskError(taskId, 'Unexpected server response. Please try again.')
      return
    }
  ...
  const handleConnectGithub = async () => {
    ...
    try {
      const res = await fetch('/api/user/connect-github')
      const data = await res.json()
      if (data.needs_linking && data.url) {
        window.location.href = data.url
        return
      }
      if (data.success) {
        setGithubUsername(data.github_username)
        toast.success(`GitHub connected: @${data.github_username}`)
      } else {
        toast.error(data.message ?? 'Could not connect GitHub.')
      }
    } catch {
  ...
  // Group levels by city for skills section
  const levelsByCity = completedLevels.reduce(
    (acc, level) => {
      if (!acc[level.city_id]) {
        acc[level.city_id] = { cityTitle: level.city_title, cityColor: level.city_color, levels: [] }
      }
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="relative">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h1 className="font-black text-white text-xl sm:text-2xl tracking-tight truncate">
                        {user.name}
                      </h1>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent
                              side="top"
                              className="bg-[#111] border border-[#1F1F1F] text-white text-xs max-w-[180px]"
                            >
              ...
              <div className="bg-[#111] border border-[#1F1F1F] rounded-sm p-5 sm:p-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {(cycleEndsIn.hours > 0 || cycleEndsIn.minutes > 0) && (
                      <span className="text-[#555] text-xs">
                        Reset in {cycleEndsIn.hours}h {cycleEndsIn.minutes}m
                      </span>
                    )}
                  </div>
                </div>
                  ...
                  {tasks.map((task) => {
                      ...
                      return (
                        <div key={task.id} className="bg-[#0D0D0D] border border-[#1A1A1A] rounded-sm p-3">
                          ...
                          {task.unlock_after_city_slug && (
                            <p className="text-[10px] text-[#2A2A2A] mt-1 ml-5">
                    ...
                    if (task.is_done_today) {
                      return (
                        <div
                          key={task.id}
                          className="bg-[#0D0D0D] rounded-sm p-3"
                    ...
                    return (
                      ...
                      >
                        ...
                        {task.description && (
                          <p className="text-[#555] text-xs mt-0.5">{task.description}</p>
                        )}

                        {/* ── study_time ── */}
                        {task.type === 'study_time' && (
                          <>
                            {task.study_minutes_today !== undefined && (
                              <div className="mt-3">
                            ...
                            {taskErrors[task.id] && (
                              <p className="text-rose-400 text-xs mt-2">{taskErrors[task.id]}</p>
                            )}
                          </>
                        )}
                        ...
                        {task.type === 'github' && (
                          <div className="mt-3">
                            {/* Connect GitHub banner (always shown if not connected) */}
                            {!githubUsername && (
                              <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-sm p-3 mb-3">
                                <p className="text-[#A0A0A0] text-xs mb-1.5">
                                  Connect your GitHub for author verification
                                </p>
                                <button
                                ...
                                <p className="text-[#444] text-[10px] mt-1.5">
                                  Without connecting, commits are still verified as real and recent.
                                </p>
                              </div>
                            )}
                            ...
                            {/* 3-field dialog trigger */}
                            <CommitDialog
                              taskId={task.id}
                              lockedUsername={githubUsername}
                              onSuccess={() =>
                                setTasks(prev =>
                                  prev.map(t =>
                                    t.id === task.id ? { ...t, is_done_today: true } : t
                                  )
                                )
                              }
                        ...
                        {/* ── linkedin / dsa ── */}
                        {(task.type === 'linkedin' || task.type === 'dsa') && (
                          <div className="mt-3">
                            <input
                              type="url"
                              placeholder={
                                task.type === 'linkedin' ? 'linkedin.com/posts/…' : 'Paste URL here…'
                              }
                            ...
                            {taskErrors[task.id] && (
                              <p className="text-rose-400 text-xs mt-2 leading-relaxed">
                                {taskErrors[task.id]}
                              </p>
                            )}
                          </div>
                ...
                {allTasksComplete && tasks.filter(t => t.is_unlocked).length > 0 && (
                  <div className="mt-4 bg-[#F97316]/10 border border-[#F97316]/30 rounded-sm p-3 flex items-center justify-center gap-2">
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  {totalLevelsMastered > 0 && (
                    <span className="text-[10px] text-emerald-400 font-semibold">
                      {totalLevelsMastered} mastered
                    </span>
                  )}
                </div>
                <p className="text-[#444] text-xs mb-5">
                  Complete levels on /world to build your skill tree.
                </p>

                {Object.keys(levelsByCity).length === 0 ? (
                  <p className="text-[#333] text-sm text-center py-6">
                    No levels started yet — head to the World Map.
                  </p>
                ) : (
                            ...
                            <div key={level.id}>
                              <div className="flex items-center gap-2 mb-1">
                                <BookOpen
                                  ...
                                  style={{ color: level.color || cityColor }}
                                />
                              ...
                              <div className="h-[2px] bg-[#1A1A1A] rounded-full overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width: `${level.completion_percent}%`,
                                    backgroundColor: level.color || cityColor,
                                  }}
                                />
                              </div>
                            </div>
                  ...
                  <div className="flex flex-col gap-3">
                    {roads.map((road) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className="w-[3px] rounded-l-sm mr-3 flex-shrink-0"
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between mt-1">
                              <p className="text-[#555] text-xs">
                                {road.completion_percent}% · {road.completed_count}/{road.total_count}
                              </p>
                              <Link
                                href="/road"
                                className="inline-flex items-center gap-1 text-[#F97316] text-xs hover:underline"
                              >
                ...
                <div className="bg-[#111] border border-[#1F1F1F] rounded-sm overflow-hidden">
                  ...
                  <div className="divide-y divide-[#141414]">
                    {tokenHistory.slice(0, 10).map((entry) => (
                      <div
                        key={entry.id}
                        className="px-5 py-3 flex items-center justify-between text-xs gap-2"
                      >
